...
- Question selection: nearest-difficulty-to-target among reviewed (`reviewed=1`), enabled, unanswered-in-this-session templates, tie-broken by template ID for reproducibility.

**Schema** (`db/migrations/create_tables.sql`, `002_mastery_levels.sql`): `children`, `skills`, `content_templates` (versioned, `reviewed`/`enabled` gates), `sessions` (tracks `current_question_id`), `attempts` (one row per session+template, `outcome` in `answered`/`skipped`), `mastery` (one row per child+skill), `cache`, `safety_events`.
